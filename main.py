...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default="mlp", choices=["lp", "mlp", "cnu"])
    parser.add_argument('--bias', type=str, default="true")
    parser.add_argument('--watch', type=str, default="false")
    parser.add_argument('--hidden', type=int, default=10)
    parser.add_argument('--benchmark', type=str, default="bi-modals_IID", choices=["bi-modals_IID",
                                                                                   "bi-modals_CI",
                                                                                   "bi-modals_CDI",
                                                                                   "bi-modals_CDID",
                                                                                   "bi-moons_IID",
    ...
    parser.add_argument('--competitor', type=str,
                        default="None")  # ER_reservoir, ER-random, GDumb, Ensemble, MIR, AGEM
    ...
    # filtering out not admissible configurations (only cnu)

    if "cnu" in args_cmd.model:
    ...
    if args_cmd.benchmark != 'imagenet':
        ...
        ds = Mini_ImageNet_CDID(dataset_params, debug=False, save_cache=True, load_cache=True, device=device)
    ...
    if "lp" not in args_cmd.model and args_cmd.draw_plots:
        with torch.no_grad():
            ...
            X_train_K, y_train_K, class_dict_K = toy.utils_toy_datasets.add_keys_from_cnu(net, X_train,
                                                                                          y_train,
                                                                                          class_dict)
    ...
    if "lp" not in args_cmd.model and args_cmd.draw_plots:
        ...
        # getting keys, if any, adding them to the data, with an ad-hoc label
        X_train_K, y_train_K, class_dict_K = d2d.utils_toy_datasets.add_keys_from_cnu(net, X_train,
                                                                                      y_train,
                                                                                      class_dict)
        ...
        # getting keys, if any, adding them to the data, with an ad-hoc label
        X_train_K, y_train_K, class_dict_K = toy.utils_toy_datasets.add_keys_from_cnu(net, normed_data,
                                                                                      y_train,
                                                                                      class_dict)
